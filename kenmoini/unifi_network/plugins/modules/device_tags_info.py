...
DOCUMENTATION = '''
---
module: device_tags_info
short_description: Returns the list of Device Tags from Unifi Network
version_added: "1.0.0"
description:
...
EXAMPLES = '''
# Get the List of Device Tags running on the Unifi Network for a Site
- name: Get Device Tags at a Site from Unifi Network
  ...
  register: r_device_tags_info
'''

RETURN = '''
device_tags_info:
    description: The data returned about the list of Device Tags at the Site managed by this Unifi Network Application
    type: object
    returned: always
'''
...
def run_module():
    ...
    if response.status_code != 200:
        check_response_errors(module, response, result, context=' while retrieving Device Tags Info')

    result['device_tags_info'] = response.json()
