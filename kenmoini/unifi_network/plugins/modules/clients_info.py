...
---
module: clients_info
short_description: Returns the list of Connected Clients from Unifi Network
version_added: "1.0.0"
description:
    - Retrieves information about the list of Clients connected to devices managed by this Unifi Network Application.
...
options:
  ...
  unifi_network_site_id:
    description:
      - The Site UUID to query for connected clients.
    required: true
...
EXAMPLES = '''
# Get the List of Connected Clients from the Unifi Network for a Site
- name: Get Connected Clients at a Site from Unifi Network
  ...
  register: r_clients_info
'''

RETURN = '''
clients_info:
    description: The data returned about the list of Connected Clients at the Site managed by this Unifi Network Application
    type: object
    returned: always
'''
...
def run_module():
    ...
    targetURL = module.params['unifi_network_url'] + apiBaseURL + '/v1/sites/' + module.params['unifi_network_site_id'] + '/clients'

    # Perform the API request to get the Connected Clients Info
    response = requests.get(targetURL, headers=headers, verify=not module.params['unifi_network_skip_tls_verify'])
    if response.status_code != 200:
        module.fail_json(msg='Failed to retrieve Connected Clients Info from Unifi Network', **result)

    result['clients_info'] = response.json()
