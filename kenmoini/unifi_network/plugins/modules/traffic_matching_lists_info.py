...
DOCUMENTATION = '''
---
module: traffic_matching_lists_info
short_description: Returns the list of Traffic Matching Lists from Unifi Network
version_added: "1.0.0"
description:
    - Retrieves information about the list of Traffic Matching Lists managed by this Unifi Network Application.
...
EXAMPLES = '''
# Get the List of Traffic Matching Lists from the Unifi Network for a Site
- name: Get Traffic Matching Lists at a Site from Unifi Network
  ...
  register: r_traffic_matching_lists_info
'''

RETURN = '''
traffic_matching_lists_info:
    description: The data returned about the list of Traffic Matching Lists at the Site managed by this Unifi Network Application
    type: object
    returned: always
'''
...
def run_module():
    ...
    targetURL = module.params['unifi_network_url'] + apiBaseURL + '/v1/sites/' + module.params['unifi_network_site_id'] + '/traffic-matching-lists?' + query_params_str

    # Apply any filters to the request URL
    targetURL = filter_requests(module, targetURL, result)

    # Perform the API request to get the Traffic Matching Lists Info
    response = requests.get(targetURL, headers=headers, verify=not module.params['unifi_network_skip_tls_verify'])
    if response.status_code != 200:
        check_response_errors(module, response, result, context=' while retrieving Traffic Matching Lists Info')

    result['traffic_matching_lists_info'] = response.json()
