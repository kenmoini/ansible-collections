...
DOCUMENTATION = '''
---
module: dpi_applications_info
short_description: Returns the list of DPI Applications from Unifi Network
version_added: "1.0.0"
description:
    - Returns predefined Deep Packet Inspection (DPI) Applications used for traffic identification and filtering.
...
EXAMPLES = '''
# Get the List of Deep Packet Inspection (DPI) Applications on the Unifi Network for a Site
- name: Get Deep Packet Inspection (DPI) Applications
  kenmoini.unifi_network.dpi_applications_info:
    unifi_network_url: https://unifi.example.com
    unifi_network_api_key: 1234567890
  register: r_dpi_applications_info
'''

RETURN = '''
dpi_applications_info:
    description: The data returned about the list of Deep Packet Inspection (DPI) Applications at the Site managed by this Unifi Network Application
    type: object
    returned: always
'''
...
def run_module():
    ...
    if response.status_code != 200:
        check_response_errors(module, response, result, context=' while retrieving DPI Applications Info')

    result['dpi_applications_info'] = response.json()
