...
DOCUMENTATION = '''
---
module: acl_rules_info
short_description: Returns the list of ACL Rules from Unifi Network
version_added: "1.0.0"
description:
    - Retrieves information about the list of ACL Rules managed by this Unifi Network Application.
...
EXAMPLES = '''
# Get the List of ACL Rules from the Unifi Network for a Site
- name: Get ACL Rules at a Site from Unifi Network
  ...
  register: r_acl_rules_info
'''

RETURN = '''
acl_rules_info:
    description: The data returned about the list of ACL Rules at the Site managed by this Unifi Network Application
    type: object
    returned: always
'''
...
def run_module():
    ...
    # Perform the API request to get the ACL Rules Info
    response = requests.get(targetURL, headers=headers, verify=not module.params['unifi_network_skip_tls_verify'])
    if response.status_code != 200:
        check_response_errors(module, response, result, context=' while retrieving ACL Rules Info')

    result['acl_rules_info'] = response.json()
