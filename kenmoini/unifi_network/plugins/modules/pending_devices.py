...
DOCUMENTATION = '''
---
module: pending_devices
short_description: Returns the list of Devices pending to be adopted from Unifi Network
version_added: "1.0.0"
description:
    - Retrieves information about the list of Devices pending to be adopted by this Unifi Network Application.
...
- name: Get Pending Devices at a Site from Unifi Network
  kenmoini.unifi_network.pending_devices:
    unifi_network_url: https://unifi.example.com
    unifi_network_api_key: 1234567890
  register: r_pending_devices
'''

RETURN = '''
pending_devices:
    description: The data returned about the list of pending devices at the Site managed by this Unifi Network Application
    type: object
    returned: always
'''
...
def run_module():
    ...
    targetURL = module.params['unifi_network_url'] + apiBaseURL + '/v1/pending-devices?limit=' + str(module.params['query_limit']) + '&offset=' + str(module.params['query_offset'])

    # Perform the API request to get the list of Pending Devices
    response = requests.get(targetURL, headers=headers, verify=not module.params['unifi_network_skip_tls_verify'])
    if response.status_code != 200:
        check_response_errors(module, response, result, context=' while retrieving Pending Devices Info')

    result['pending_devices'] = response.json()
