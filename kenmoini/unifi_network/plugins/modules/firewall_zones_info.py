...
DOCUMENTATION = '''
---
module: firewall_zones_info
short_description: Returns the list of Firewall Zones from Unifi Network
version_added: "1.0.0"
description:
    - Retrieves information about the list of Firewall Zones managed by this Unifi Network Application.
...
EXAMPLES = '''
# Get the List of Firewall Zones from the Unifi Network for a Site
- name: Get Firewall Zones at a Site from Unifi Network
  ...
  register: r_firewall_zones_info
'''

RETURN = '''
firewall_zones_info:
    description: The data returned about the list of Firewall Zones at the Site managed by this Unifi Network Application
    type: object
    returned: always
'''
...
def run_module():
    ...
    targetURL = module.params['unifi_network_url'] + apiBaseURL + '/v1/sites/' + module.params['unifi_network_site_id'] + '/firewall/zones?' + query_params_str
    ...
    if response.status_code != 200:
        check_response_errors(module, response, result, context=' while retrieving Firewall Zones Info')

    result['firewall_zones_info'] = response.json()
