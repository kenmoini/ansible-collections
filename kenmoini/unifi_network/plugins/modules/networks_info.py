...
short_description: Returns the list of Networks from Unifi Network
version_added: "1.0.0"
description:
    - Retrieves information about the list of Networks managed by this Unifi Network Application.
...
EXAMPLES = '''
# Get the List of Networks from the Unifi Network for a Site
- name: Get Networks at a Site from Unifi Network
...
RETURN = '''
networks_info:
    description: The data returned about the list of Networks at the Site managed by this Unifi Network Application
    type: object
    returned: always
'''
...
def run_module():
    ...
    targetURL = module.params['unifi_network_url'] + apiBaseURL + '/v1/sites/' + module.params['unifi_network_site_id'] + '/networks?' + query_params_str

    # Apply any filters to the request URL
    targetURL = filter_requests(module, targetURL, result)

    # Perform the API request to get the Networks Info
    response = requests.get(targetURL, headers=headers, verify=not module.params['unifi_network_skip_tls_verify'])
    if response.status_code != 200:
        check_response_errors(module, response, result, context=' while retrieving Networks Info')

    result['networks_info'] = response.json()
